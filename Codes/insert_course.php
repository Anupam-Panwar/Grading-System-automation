...
if (isset($_POST['course_name']) && isset($_POST['code']) && isset($_POST['year']) && isset($_POST['semester']) && isset($_GET['id'])) {
    ...
    function setGrade($cd, $gn, $ub, $lb, $conn, $id)
    {
        ...
        if ($conn->query($sql) !== TRUE) {
            header('Location:teacher_courses.php?error=Unexpected Input&id=' . $id);
        }
    }
    ...
    if (empty($course_name)) {
        header('Location: teacher_courses.php?error=Course Name required&id=' . $teacher_id);
        exit();
    } else if (empty($code)) {
        header('Location: teacher_courses.php?error=Course Code Required&id=' . $teacher_id);
        exit();
    } else if (empty($year)) {
        header('Location: teacher_courses.php?error=Year value required&id=' . $teacher_id);
        exit();
    } else if (empty($semester)) {
        header('Location: teacher_courses.php?error=Semester value required!&id=' . $teacher_id);
        exit();
    } else if (empty($teacher_id)) {
        header('Location: teacher_courses.php?error=There was an error!&id=' . $teacher_id);
        exit();
    } else if (empty($department)) {
        header('Location: teacher_courses.php?error=There was an error-department!&id=' . $teacher_id);
        exit();
    } else if (empty($type)) {
        header('Location: teacher_courses.php?error=There was an error-type!&id=' . $teacher_id);
        exit();
    } else {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            header('Location:teacher_courses.php?error=Successfully added Course&id=' . $teacher_id);
            exit();
        } else {
            header('Location:teacher_courses.php?error=Unexpected Course&id=' . $teacher_id);
            exit();
        } 
    }
} else {

    header('Location:teacher_courses.php?error=Enter data&id=' . $teacher_id);
    exit();
}
